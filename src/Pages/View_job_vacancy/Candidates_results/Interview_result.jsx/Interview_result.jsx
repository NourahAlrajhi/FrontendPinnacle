...
/* cloudinary.config({
  ...
  const OnChoosenQuestion = async (ID) => {
    ...
    <>
      ...
      <Box className="Interview_result_box">
        ...
        <Box>
          ...
          {/* passed */}
          <Box component="span" sx={{ display: "inline-block", margin: "10px 0", padding: "5px 15px", background: CandidateResultr == "Passed" ? "#DEF8EE" : "red", fontWeight: "600", color: CandidateResultr == "Passed" ? "#4AA785" : "white", borderRadius: "10px" }}>{CandidateResultr}</Box>
          {/* Not Attended */}
        ...
        <Box mt={5}>
          ...
          <Grid item xs={12}>
            ...
            >
              ...
              >
                {QUESTIONS && QUESTIONS.map((option) => (
                  <MenuItem key={option.id} value={option.id} onClick={() => OnChoosenQuestion(option.id)}>
                    {option.questions}
                  </MenuItem>
                ))}
              </TextField>
